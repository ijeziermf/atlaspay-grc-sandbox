#!/usr/bin/env python3
"""
phase0c_matrix_fix.py — populate the existing AtlasPay 5x5 RiskMatrix with a
valid 5x5 ISO-27005 json_definition.

Phase 0a created the matrix via Django ORM (REST writes are broken for
ReferentialSerializer-derived models in CA v3.18.3 OSS), but the
json_definition was left empty. That breaks risk-scenarios: their
inherent_level and residual_level fields all evaluate to -1 because the
matrix can't be looked up.

This script patches the matrix in-place via Django ORM. It does NOT
create a new matrix — it populates the existing one so existing FK
references from RiskAssessment + risk-scenarios stay valid.

Run:
    docker compose -f ~/Documents/IfeSec/Tools/ciso-assistant-community/docker-compose.yml \
        exec -T backend python /scripts/phase0c_matrix_fix.py
"""
...
MATRIX_NAME = "AtlasPay 5x5"

# Canonical 5x5 ISO-27005 definition, transcribed verbatim from
# backend/library/libraries/risk-matrix-5x5-iso-27005.yaml so we know the
# schema is what CA v3.18.3 expects. Only English translations are included
# here to keep the payload small; full multi-locale isn't needed for a
# sandbox portfolio expansion.
DEFINITION = {
    "probability": [
    ...
    # 5x5 grid: rows = probability (0..4), cols = impact (0..4)
    # Cell value = risk level id (0..4).
    "grid": [
        [0, 0, 1, 1, 1],
        [0, 1, 1, 2, 2],
        [1, 1, 2, 3, 3],
